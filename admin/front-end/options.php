<div class="wrap" id="of_container">
	...
	<span style="display: none;" id="hooks"><?php echo json_encode(
     of_get_header_classes_array()
 ); ?></span>
	<input type="hidden" id="reset" value="<?php if (isset($_REQUEST["reset"])) {
     echo $_REQUEST["reset"];
 } ?>" />
	<input type="hidden" id="security" name="security" value="<?php echo wp_create_nonce(
     "of_ajax_nonce"
 ); ?>" />
	<form id="of_form" method="post" action="<?php echo esc_attr(
     $_SERVER["REQUEST_URI"]
...
?>
		...
		<div class="save_bar"> 
			...
			<img style="display:none" src="<?php echo ADMIN_DIR; ?>assets/images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
			<button id ="of_save" type="button" class="button-primary"><?php _e(
       "Save All Changes"
   ); ?></button>			
			<button id ="of_reset" type="button" class="button submit-button reset-button" ><?php _e(
       "Options Reset"
   ); ?></button>
