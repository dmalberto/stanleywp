...
if (!class_exists("RWMB_Field ")) {
    class RWMB_Field
    {
        ...
        static function show($field, $saved)
        {
            global $post;
            $meta = call_user_func(
                [RW_Meta_Box::get_class_name($field), "meta"],
                $post->ID,
                $saved,
                $field
            );
            ...
            $type = $field["type"];
            $id = $field["id"];
            $begin = call_user_func(
                [RW_Meta_Box::get_class_name($field), "begin_html"],
                $meta,
                $field
            );
            ...
            $begin = apply_filters(
                "rwmb_{$type}_begin_html",
                $begin,
                $field,
                $meta
            );
            $begin = apply_filters(
                "rwmb_{$id}_begin_html",
                $begin,
                $field,
                $meta
            );
            ...
            if ($field["clone"]) {
                ...
                foreach ($meta as $index => $sub_meta) {
                    ...
                    $input_html .= call_user_func(
                        [RW_Meta_Box::get_class_name($sub_field), "html"],
                        $sub_meta,
                        $sub_field
                    );
            ...
            // Non-cloneable fields
                ...
                // Call separated methods for displaying each type of field
                $field_html = call_user_func(
                    [RW_Meta_Box::get_class_name($field), "html"],
                    $meta,
                    $field
                );
                // Apply filter to field HTML
            ...
            $end = call_user_func(
                [RW_Meta_Box::get_class_name($field), "end_html"],
                $meta,
                $field
            );
            ...
            // 2nd filter applies to current field only
            $html = apply_filters(
                "rwmb_{$type}_wrapper_html",
                "{$begin}{$field_html}{$end}",
                $field,
                $meta
            );
            $html = apply_filters(
                "rwmb_{$id}_wrapper_html",
                $html,
                $field,
                $meta
            );
            // Display label and input in DIV and allow user-defined classes to be appended
            $classes = ["rwmb-field", "rwmb-{$type}-wrapper"];
        ...
        static function end_html($meta, $field)
        {
            ...
            if ($field["clone"]) {
                $button =
                    '<a href="#" class="rwmb-button button-primary add-clone">' .
                    __("+", "rwmb") .
                    "</a>";
            }
            $desc = !empty($field["desc"])
        ...
        {
            return '<a href="#" class="rwmb-button button remove-clone">' .
                __("&#8211;", "rwmb") .
                "</a>";
        }
        /**
