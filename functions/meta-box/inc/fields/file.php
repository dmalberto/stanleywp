...
if (!class_exists("RWMB_File_Field")) {
    class RWMB_File_Field extends RWMB_Field
    {
        ...
        static function admin_enqueue_scripts()
        {
            ...
            wp_localize_script("rwmb-file", "rwmbFile", [
                "maxFileUploadsSingle" => __(
                    "You may only upload maximum %d file",
                    "rwmb"
                ),
                "maxFileUploadsPlural" => __(
        ...
        static function add_actions()
        {
            ...
            add_action("wp_ajax_rwmb_delete_file", [
                __CLASS__,
                "wp_ajax_delete_file",
            ]);
        }
        ...
        {
            $i18n_title = apply_filters(
                "rwmb_file_upload_string",
                _x("Upload Files", "file upload", "rwmb"),
                $field
            );
            $i18n_more = apply_filters(
                "rwmb_file_add_string",
                _x("+ Add new file", "file upload", "rwmb"),
                $field
            );
            // Uploaded files
            $html = self::get_uploaded_files($meta, $field);
            $new_file_classes = ["new-files"];
            if (
                !empty($field["max_file_uploads"]) &&
                count($meta) >= (int) $field["max_file_uploads"]
            ) {
        ...
        static function get_uploaded_files($files, $field)
        {
            ...
            $ol =
                '<ul class="%s" data-field_id="%s" data-delete_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s" data-mime_type="%s">';
            $html = sprintf(
                $ol,
                implode(" ", $classes),
                $field["id"],
        ...
        {
            $i18n_delete = apply_filters(
                "rwmb_file_delete_string",
                _x("Delete", "file upload", "rwmb")
            );
            $i18n_edit = apply_filters(
                "rwmb_file_edit_string",
                _x("Edit", "file upload", "rwmb")
            );
            $li = '
			<li>
            ...
            return sprintf(
                $li,
                wp_get_attachment_image($attachment_id, [60, 60], true),
        ...
        static function value($new, $old, $post_id, $field)
        {
            ...
            foreach ($files as $file_item) {
                $file = wp_handle_upload($file_item, ["test_form" => false]);
                if (!isset($file["file"])) {
                    continue;
                }
                $file_name = $file["file"];
                ...
                if (!is_wp_error($id)) {
                    wp_update_attachment_metadata(
                        $id,
                        wp_generate_attachment_metadata($id, $file_name)
                    );
