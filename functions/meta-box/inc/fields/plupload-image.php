...
if (!class_exists("RWMB_Plupload_Image_Field")) {
    class RWMB_Plupload_Image_Field extends RWMB_Image_Field
    {
        ...
        {
            parent::add_actions();
            add_action("wp_ajax_rwmb_plupload_image_upload", [
                __CLASS__,
                "handle_upload",
            ]);
        }
        ...
        {
            global $wpdb;
            $post_id = is_numeric($_REQUEST["post_id"])
                ? $_REQUEST["post_id"]
                : 0;
            ...
            $file_attr = wp_handle_upload($file, ["test_form" => false]);
            //Get next menu_order
            ...
            if (!is_wp_error($id)) {
                wp_update_attachment_metadata(
                    $id,
                    wp_generate_attachment_metadata($id, $file_attr["file"])
                );
        ...
        static function admin_enqueue_scripts()
        {
            ...
            wp_enqueue_style(
                "rwmb-plupload-image",
                RWMB_CSS_URL . "plupload-image.css",
                ["wp-admin"],
                RWMB_VER
            );
            wp_enqueue_script(
                "rwmb-plupload-image",
                RWMB_JS_URL . "plupload-image.js",
                ["jquery-ui-sortable", "wp-ajax-response", "plupload-all"],
                RWMB_VER,
                true
            );
            wp_localize_script("rwmb-plupload-image", "RWMB", [
                "url" => RWMB_URL,
            ]);
        }
        /**
        ...
        static function html($meta, $field)
        {
            ...
            $i18n_drop = apply_filters(
                "rwmb_plupload_image_drop_string",
                _x("Drop images here", "image upload", "rwmb"),
                $field
            );
            $i18n_or = apply_filters(
                "rwmb_plupload_image_or_string",
                _x("or", "image upload", "rwmb"),
                $field
            );
            $i18n_select = apply_filters(
                "rwmb_plupload_image_select_string",
                _x("Select Files", "image upload", "rwmb"),
                $field
            );
            // Uploaded images
