    ...
    class RWMB_Taxonomy_Field extends RWMB_Field
    {
        ...
        static function admin_enqueue_scripts()
        {
            ...
            wp_enqueue_script(
                ...
                ["jquery", "rwmb-select-advanced", "wp-ajax-response"],
                RWMB_VER,
                true
            );
        }
        ...
        static function normalize_field($field)
        {
            ...
            // Set default args
            $field["options"]["args"] = !isset($field["options"]["args"])
                ? $default_args
                : wp_parse_args($field["options"]["args"], $default_args);
            $tax = get_taxonomy($field["options"]["taxonomy"]);
            $field["placeholder"] = empty($field["placeholder"])
                ? sprintf(
                    __("Select a %s", "rwmb"),
            ...
            switch ($field["options"]["type"]) {
                case "select_advanced":
                    $field = RWMB_Select_Advanced_Field::normalize_field(
                        $field
                    );
                    break;
                case "checkbox_list":
         ...
         * @return string
         */
        static function walk_checkbox_tree(
            $meta,
            $field,
            $elements,
            $parent = 0,
            ...
            foreach ($terms as $term) {
                ...
                $html .=
                    self::walk_checkbox_tree(
                        $meta,
                        $field,
                        $elements,
                        $term->term_id,
                        $active && in_array($term->term_id, $meta)
                    ) . "</li>";
            }
            $html .= "</ul>";
            return $html;
         ...
         * @return string
         */
        static function walk_select_tree(
            $meta,
            $field,
            $elements,
            $parent = 0,
         ...
         * @return array
         */
        static function get_options($terms = [])
        {
            $options = [];
            foreach ($terms as $term) {
