...
if (!class_exists("RWMB_Helper")) {
    ...
    class RWMB_Helper
    {
        ...
        {
            add_shortcode("rwmb_meta", [__CLASS__, "shortcode"]);
        }
        /**
        ...
        static function shortcode($atts)
        {
            ...
            if (in_array($atts["type"], ["file", "file_advanced"])) {
                $content = "<ul>";
            ...
            elseif (is_array($meta)) {
                $content =
                    "<ul><li>" . implode("</li><li>", $meta) . "</li></ul>";
            } else {
                $content = $meta;
            }
         ...
         * @return mixed
         */
        static function meta($key, $args = [], $post_id = null)
        {
            ...
            if (!isset($args["multiple"])) {
                $args["multiple"] = in_array($args["type"], [
                    "checkbox_list",
                    "file",
                    "file_advanced",
                    "image",
            ...
            // Get uploaded files info
            if (in_array($args["type"], ["file", "file_advanced"])) {
            ...
            elseif ("taxonomy" == $args["type"]) {
                $meta = empty($args["taxonomy"])
                    ? []
                    : wp_get_post_terms($post_id, $args["taxonomy"]);
            }
            // Get map
         ...
         * @return array|bool False if file not found. Array of (id, name, path, url) on success
         */
        static function image_info($id, $args = [])
        {
            $args = wp_parse_args($args, [
                "size" => "thumbnail",
            ]);
         ...
         * @return string
         */
        static function map($key, $args = [], $post_id = null)
        {
            $post_id = empty($post_id) ? get_the_ID() : $post_id;
