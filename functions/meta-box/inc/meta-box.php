    ...
    class RW_Meta_Box
    {
        ...
        function __construct($meta_box)
        {
            ...
            foreach ($fields as $field) {
                call_user_func([self::get_class_name($field), "add_actions"]);
            }
            // Add meta box
        ...
        function admin_enqueue_scripts()
        {
            ...
            if ($this->validation) {
                wp_enqueue_script(
                    "jquery-validate",
                    RWMB_JS_URL . "jquery.validate.min.js",
                    ["jquery"],
                    RWMB_VER,
                    true
                );
        ...
        function show()
        {
            ...
            wp_nonce_field(
                "rwmb-save-{$this->meta_box["id"]}",
                "nonce_{$this->meta_box["id"]}"
            );
            ...
            foreach ($this->fields as $field) {
                call_user_func(
                    [self::get_class_name($field), "show"],
                    $field,
                    $saved
                );
            }
            ...
            if (isset($this->validation) && $this->validation) {
                echo '
					...
					{
						var rwmb = {
							validationOptions : jQuery.parseJSON( \'' .
                    json_encode($this->validation) .
                    '\' ),
							summaryMessage : "' .
                    __(
					...
					{
						var tempOptions = jQuery.parseJSON( \'' .
                    json_encode($this->validation) .
                    '\' );
        ...
        function save_post($post_id)
        {
            ...
            $id = $this->meta_box["id"];
            if (
                empty($_POST["nonce_{$id}"]) ||
                !wp_verify_nonce($_POST["nonce_{$id}"], "rwmb-save-{$id}")
            ) {
                return;
            }
            // Autosave
            ...
            do_action(
                "rwmb_{$this->meta_box["id"]}_before_save_post",
                $post_id
            );
            foreach ($this->fields as $field) {
                $name = $field["id"];
                ...
                $new = call_user_func(
                    [self::get_class_name($field), "value"],
                    $new,
                    $old,
                    $post_id,
                    $field
                ...
                call_user_func(
                    [self::get_class_name($field), "save"],
                    $new,
                    $old,
                    $post_id,
                    $field
        ...
        static function normalize_fields($fields)
        {
            foreach ($fields as &$field) {
                ...
                $field = call_user_func(
                    [self::get_class_name($field), "normalize_field"],
                    $field
                );
                if (isset($field["fields"])) {
